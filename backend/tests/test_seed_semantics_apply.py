...
def test_allow_name_match_false_keeps_when_other_terms_present() -> None:
    ...
    items = [
        {"parent": "dogs", "matches": 50},  # should be dropped (name-only)
        {
            "parent": "dogs wif",
            "matches": 60,
        },  # should be kept (name + another term)
        {
            "parent": "blocked token",
            "matches": 99,
        },  # should be dropped by blocklist
    ]
...
def test_require_all_terms_positive_branch_keeps_item_with_all_terms() -> None:
