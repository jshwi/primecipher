...
#   TOL_MIN (default 20)
#   DB_PATH (optional)      # sqlite file to use for both API and tool
...
# Pin one absolute sqlite path for BOTH uvicorn and the backfill tool
...
command -v sqlite3 >/dev/null 2>&1 || log "sqlite3 not found (optional)"
...
if ! is_up; then
  ...
  export PYTHONPATH=.
  # Make sure directory for DB exists (when pointing into backend/)
  DB_DIR="$(dirname "${PRIMECIPHER_DB_PATH}")"
  mkdir -p "${DB_DIR}"

  # Start uvicorn (inherits PRIMECIPHER_DB_PATH)
...
# Show DB path used (useful when debugging)
...
# Run the backfill tool with the SAME DB (PRIMECIPHER_DB_PATH already exported)
PYTHONPATH="${REPO_ROOT}/backend" python -m app.tools.synthetic_backfill \
  --window "${HOLD}" --narrative "${NARRATIVE}" --parent "${PARENT}"
...
# Expect legacy response shape: .summary.n_trades present
...
echo "$WALK_JSON" | jq '.summary, .diagnostics' >/dev/null || true

if [[ "$WALK_TRADES" -lt 1 ]]; then
  # Helpful debug if sqlite is available
